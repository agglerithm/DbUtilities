...
namespace DbUtilities.DataTransformation
{
    public abstract class FieldReader
    {
        ...
        protected TypeCode GetTypeCode(int ordinal)
        {
        ...
        public static MethodInfo GetReaderMethod(Type type)
        {
            ...
            mi = type.IsNullableType() ? _miReadNullableValue.MakeGenericMethod(type.GetNonNullableType()) : _miReadValue.MakeGenericMethod(type);
            return mi;
        }
    ...
    public class DbFieldReader : FieldReader
    {
        ...
        public DbFieldReader(DbDataReader reader, Action<object> logger = null)
        {
            this._reader = reader;
            _logger = logger;
            this.Init();
        ...
        protected override int FieldCount => this._reader.FieldCount;
        ...
        {
            var val =_reader.GetValue(ordinal);
            if (val.GetType().Name == "DBNull")
                return default(T);
            if (!val.GetType().GetNullAssignableType().IsAssignableFrom(typeof(T)))
                throw new Exception($"Cannot convert from type {typeof(T).Name} to type {val.GetType().Name}.");
            return (T) val;
        }
        ...
        public object Read(int ordinal, Type propType)
        {
            var propName = GetNullTypeName(propType);
            switch (propName)
            {
                case "bool":
                case "Boolean":
                    return propType.IsNullableType() ? GetValue<bool?>(ordinal) : GetValue<bool>(ordinal);
                case "Int16":
                case "short":
                    return propType.IsNullableType() ? ReadNullableInt16(ordinal) : GetInt16(ordinal);
                case "Int32":
                case "int":
                    return propType.IsNullableType() ? ReadNullableInt32(ordinal) : ReadInt32(ordinal);
                case "Int64":
                case "long":
                    return propType.IsNullableType() ? ReadNullableInt64(ordinal) : ReadInt64(ordinal);
                case "byte":
                case "Byte":
                    return propType.IsNullableType() ? ReadNullableByte(ordinal) : ReadByte(ordinal);
                case "string":
                case "String":
                    return ReadString(ordinal);
                case "Single":
                    return propType.IsNullableType() ? ReadNullableSingle(ordinal) : ReadSingle(ordinal);
                case "Double":
                case "float":
                    return propType.IsNullableType() ? ReadNullableDouble(ordinal) : ReadDouble(ordinal);
                case "Decimal":
                    return propType.IsNullableType() ? ReadNullableDecimal(ordinal) : ReadDecimal(ordinal);
                case "DateTime":
                    return propType.IsNullableType() ? ReadNullableDateTime(ordinal) : ReadDateTime(ordinal);
                case "Guid":
                    return propType.IsNullableType() ? ReadNullableGuid(ordinal) : ReadGuid(ordinal);
                case "char":
                case "Char":
                    return propType.IsNullableType() ? ReadNullableChar(ordinal) : ReadChar(ordinal);
                default:
                    throw new Exception($"The type {propType} is not supported.");
            }
        }

        private string GetNullTypeName(Type propType)
        {
            return propType.Name != "Nullable`1" ? propType.Name : propType.GenericTypeArguments.First().Name;
        }


        public int GetOrdinalByName(string propName)
        {
            var ordinal = -1;
            try
            {
                ordinal = _reader.GetOrdinal(propName);
            }
            catch (Exception ex)
            {
                throw new Exception($"Could not resolve ordinal for property name {propName}: {ex}");
            }
            return ordinal;
        }
